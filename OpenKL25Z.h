...
**
**     Abstract:
**         This header file implements peripheral memory map for KL25Z
**         processor.
**
 ...
 * @version 1.2
 * @date 2012-06-21
 * @brief Peripheral memory map for KL25Z
 *
 * This header file implements peripheral memory map for KL25Z processor.
 */
#ifndef _OPENKL25Z_h_
#define _OPENKL25Z_h_
...
#if !defined(MCU_KL25Z)  /* Check if memory map has not been already included */
#define MCU_KL25Z

/* Check if another memory map has not been also included */
#if (defined(MCU_ACTIVE))
  #error KL25Z memory map: There is already included another memory map. Only one memory map can be included.
...
typedef struct ADC_MemMap {
  uint32_t SC1[2];      /**< ADC Status and Control Registers 1, array offset: 0x0, array step: 0x4. See \ref KL25Z_ADC_SC1A_Bit_Fields. */
...
/** 
    \addtogroup KL25Z_ADC_Register_Bit_Fields_Mask ADC Register Bit Fields Mask 
    @{
 */

/**
    \addtogroup KL25Z_ADC_SC1A_Bit_Fields A/D  Status and Control Registers (ADC0_SC1A)
    \brief Defines for the bit fields in the \ref ADC0_SC1A register.
    @{
*/
/** 
 * Input channel select (4-0 bits).
 * Selects one of the input channels. The input channel decode depends 
 * on the value of [DIFF](\ref ADC_SC1_DIFF_MASK). DAD0-DAD3 are 
 * associated with the input pin pairs DADPx and DADMx.
 * 
 * \note Some of the input channel options in the bitfield-setting 
 * descriptions might not be available for your device. For the actual 
 * ADC channel assignments for your device, see the Chip Configuration 
 * details.
 * The successive approximation converter subsystem is turned off when 
 * the channel select bits are all set, that is, ADCH = 11111. This 
 * feature allows explicit disabling of the ADC and isolation of the 
 * input channel from all sources. Terminating continuous conversions 
 * this way prevents an additional single conversion from being 
 * performed. It is not necessary to set [ADCH](\ref ADC_SC1_ADCH_MASK) 
 * to all 1s to place the ADC in a low-power state when continuous 
 * conversions are not enabled because the module automatically enters a 
 * low-power state when a conversion completes.
 * 
 * Value | Status
 * :----:|:------
 ...
 * 11100 | Reserved.
 * 11101 | When DIFF=0,V REFSH is selected as input; when DIFF=1, -V REFSH (differential) is selected as input. Voltage reference selected is determined by SC2[REFSEL].
 * 11110 | When DIFF=0,V REFSL is selected as input; when DIFF=1, it is reserved. Voltage reference selected is determined by SC2[REFSEL].
 ...
 * Configures the ADC to operate in differential mode. When enabled, 
 * this mode automatically selects from the differential channels, and 
 * changes the conversion algorithm and the number of cycles to complete 
 * a conversion.
 * 
 * Value | Status
 * :----:|:------
...
//! Differential mode mask shift.
#define ADC_SC1_DIFF_SHIFT                       5
/** 
 * Interrupt enable. 
 * Enables conversion complete interrupts. When [COCO](\ref ADC_SC1_COCO_MASK) 
 * becomes set while the respective [AIEN](\ref ADC_SC1_AIEN_MASK) is 
...
/** 
 * Conversion complete flag. This is a read-only field that is set each 
 * time a conversion is completed when the compare function is disabled,
 * or SC2[ACFE]=0 and the hardware average function is disabled, or 
 * SC3[AVGE]=0. When the compare function is enabled, or SC2[ACFE]=1, 
 * COCO is set upon completion of a conversion only if the compare 
 * result is TRUE. When the hardware average function is enabled, or 
 * SC3[AVGE]=1, COCO is set upon completion of the selected number of 
 * conversions (determined by AVGS). COCO in SC1A is also set at the 
 * completion of a calibration sequence. COCO is cleared when the 
 * respective SC1n register is written or when the respective Rn register 
 * is read.
 * 
 * Value | Status
 * :----:|:------
...
/** Conversion shift bits. */
#define ADC_SC1_COCO_SHIFT                       0x00000007

/** @} */
/* End of SC1 Bit Fields */

/* CFG1 Bit Fields */
/**
    \addtogroup KL25Z_ADC_CFG1_Bit_Fields A/D  Status and Control Registers (ADC0_SC1A)
    \brief Defines for the bit fields in the \ref ADC0_CFG1 register.
    @{
*/
/**
 * Low-Power Configuration. (7 bit)
 * Controls the power configuration of the successive approximation 
 * converter. This optimizes power consumption when higher sample rates 
 * are not required.
 * 
 * Value | Status
 * :----:|:------
...
/** Low-Power Configuration mask shift. */
#define ADC_CFG1_ADLPC_SHIFT                     7
/**
 * Clock Divide Select. (6-5 bits)
 * ADIV selects the divide ratio used by the ADC to generate the 
 * internal clock ADCK.
 * 
 * Value | Status
 * :----:|:------
...
/** Clock Divide Select mask shift. */
#define ADC_CFG1_ADIV_SHIFT                      5
#define ADC_CFG1_ADIV(x)                         (((uint32_t)(((uint32_t)(x))<<ADC_CFG1_ADIV_SHIFT))&ADC_CFG1_ADIV_MASK)
/**
 * Sample time configuration (4 bit)
 * ADLSMP selects between different sample times based on the conversion 
 * mode selected. This bit adjusts the sample period to allow higher 
 * impedance inputs to be accurately sampled or to maximize conversion 
 * speed for lower impedance inputs. Longer sample times can also be 
 * used to lower overall power consumption if continuous conversions are 
 * enabled and high conversion rates are not required. When ADLSMP=1, 
 * the long sample time select bits, (ADLSTS[1:0]), can select the 
...
/** Sample Time Configuration mask shift. */
#define ADC_CFG1_ADLSMP_SHIFT                    4

/**
 * Conversion mode selection (3-2 bits)
...
/** Conversion mode selection mask shift. */
#define ADC_CFG1_MODE_SHIFT                      0x02
/**
 * Input Clock Select (1-0 bits)
 * Selects the input clock source to generate the internal clock, ADCK. 
 * Note that when the ADACK clock source is selected, it is not required 
 * to be active prior to conversion start. When it is selected and it is 
 * not active prior to a conversion start, when CFG2[ADACKEN]=0, the 
 * asynchronous clock is activated at the start of a conversion and 
 * deactivated when conversions are terminated. In this case, there is an 
 * associated clock startup delay each time the clock source is 
 * re-activated. 
 * 
 * Value | Status
 * :----:|:------
...
/** Input Clock Select Mask Shift. */
#define ADC_CFG1_ADICLK_SHIFT                    0
#define ADC_CFG1_ADICLK(x)                       (((uint32_t)(((uint32_t)(x))<<ADC_CFG1_ADICLK_SHIFT))&ADC_CFG1_ADICLK_MASK)
/** @} */
/* End of CFG1 Bit Fields */

/* CFG2 Bit Fields */
/**
    \addtogroup KL25Z_ADC_CFG2_Bit_Fields A/D  Status and Control Registers (ADC0_SC1A)
    ...
    configuration for very high speed conversions and selects the long 
    sample time duration during long sample mode.
    @{
*/
/**
 * ADC Mux Select (4 bit).
...
/** ADC Mux Select Mask Shift. */
#define ADC_CFG2_MUXSEL_SHIFT                    4
/**
 * Asynchronous Clock Output Enable (3 bit).
 * Enables the asynchronous clock source and the clock source output 
 * regardless of the conversion and status of CFG1[ADICLK]. Based on MCU
 * configuration, the asynchronous clock may be used by other modules. 
 * See chip configuration information. Setting this field allows the 
 * clock to be used even while the ADC is idle or operating from a 
 * different clock source. Also, latency of initiating a single or 
 * first-continuous conversion with the asynchronous clock selected is 
...
/** Asynchronous Clock Output Enable Mask Shift. */
#define ADC_CFG2_ADACKEN_SHIFT                   3
/**
 * High-Speed Configuration (2 bit).
 * Configures the ADC for very high-speed operation. The conversion 
 * sequence is altered with 2 ADCK cycles added to the conversion time 
...
/** High-Speed Configuration Mask Shift. */
#define ADC_CFG2_ADHSC_SHIFT                     2
/**
 * Long Sample Time Select (1-0 bits).
 * Selects between the extended sample times when long sample time is 
 * selected, that is, when CFG1[ADLSMP]=1. This allows higher impedance 
 * inputs to be accurately sampled or to maximize conversion speed for 
 * lower impedance inputs. Longer sample times can also be used to lower
 * overall power consumption when continuous conversions are enabled if 
 * high conversion rates are not required.
 * 
 * Value | Status
 * :----:|:------
 ...
 * 11    | extra ADCK cycles; 6 ADCK cycles total sample time.
 * 
 */
#define ADC_CFG2_ADLSTS_MASK                     0x3u
/** Long Sample Time Selec Mask Shift. */
#define ADC_CFG2_ADLSTS_SHIFT                    0
#define ADC_CFG2_ADLSTS(x)                       (((uint32_t)(((uint32_t)(x))<<ADC_CFG2_ADLSTS_SHIFT))&ADC_CFG2_ADLSTS_MASK)
/** @} */
/* End of CFG2 Bit Fiels */

/**
    \addtogroup KL25Z_ADC_Register_RA A/D Data Result Register(ADC0_RA)
    \brief Defines for the bit fields in the \ref ADC0_RA register.
    The data result registers (Rn) contain the result of an ADC 
    conversion of the channel selected by the corresponding status and 
    channel control register (SC1A:SC1n). For every status and channel 
    control register, there is a corresponding data result register. 
    Unused bits in R n are cleared in unsigned right-justified modes and
    carry the sign bit (MSB) in sign-extended 2's complement modes. For 
    example, when configured for 10-bit single-ended mode, D[15:10] are 
    cleared. When configured for 11-bit differential mode, D[15:10] 
    carry the sign bit, that is, bit 10 extended through bit 15.
    @{
*/
/** Data Result A (15-0 bits). */
#define ADC_R_D_MASK                             0xFFFFu
/** Data Result A Mask Shift. */
#define ADC_R_D_SHIFT                            0
#define ADC_R_D(x)                               (((uint32_t)(((uint32_t)(x))<<ADC_R_D_SHIFT))&ADC_R_D_MASK)
/** @} */

/**
    \addtogroup KL25Z_ADC_Register_CV1 A/D Compare Value Register(ADC0_CV1)
    \brief Defines for the bit fields in the \ref ADC0_CV1 register.
    The compare value registers (CV1 and CV2) contain a compare value 
    used to compare the conversion result when the compare function is 
    enabled, that is, SC2[ACFE]=1. This register is formatted in the 
    same way as the Rn registers in different modes of operation for 
    both bit position definition and value format using unsigned or 
    sign-extended 2's complement. Therefore, the compare function uses 
    only the CVn fields that are related to the ADC mode of operation. 
    The compare value 2 register (CV2) is used only when the compare
    range function is enabled, that is, SC2[ACREN]=1.
    @{
*/
/** Compare Value (15-0 bits). */
#define ADC_CV1_CV_MASK                          0xFFFFu
/** Compare Value Mask Shift. */
#define ADC_CV1_CV_SHIFT                         0
#define ADC_CV1_CV(x)                            (((uint32_t)(((uint32_t)(x))<<ADC_CV1_CV_SHIFT))&ADC_CV1_CV_MASK)
/** @} */

/**
    \addtogroup KL25Z_ADC_Register_CV2 A/D Compare Value Register(ADC0_CV2)
...
/**
    \addtogroup KL25Z_ADC_Register_SC2 A/D Compare Value Register(ADC0_SC2)
    ...
    active, hardware/software trigger select, compare function, and 
    voltage reference select of the ADC module.
    @{
*/
/* SC2 Bit Fields */
/**
 * Conversion Active (7 bit). 
 * Indicates that a conversion or hardware averaging is in progress. 
 * ADACT is set when a conversion is initiated and cleared when a 
...
/** Conversion Active Mask Shift. */ 
#define ADC_SC2_ADACT_SHIFT                      7
/**
 * Conversion Trigger Select (6 bit). 
 * Selects the type of trigger used for initiating a conversion. Two 
 * types of trigger are selectable:
 * * Software trigger: When software trigger is selected, a conversion 
 * is initiated following a write to SC1A.
 * * Hardware trigger: When hardware trigger is selected, a conversion 
 * is initiated following the assertion of the ADHWT input after a pulse
 * of the ADHWTSn input. 
 * 
 * Value | Status
 * :----:|:------
...
/** Conversion Trigger Select Mask Shift */
#define ADC_SC2_ADTRG_SHIFT                      6
/** 
 * Compare Function Enable (5 bit).
...
/** Compare Function Enable Mask Shift */
#define ADC_SC2_ACFE_SHIFT                       5
/**
 * Compare Function Greater Than Enable (4 bit). 
 * Configures the compare function to check the conversion result 
 * relative to the CV1 and CV2 based upon the value of ACREN. ACFE must 
...
/** Compare Function Greater Than Enable Mask Shift. */
#define ADC_SC2_ACFGT_SHIFT                      4
/**
 * Compare Function Range Enable (3 bit). 
 * Configures the compare function to check if the conversion result of 
 * the input being monitored is either between or outside the range 
 * formed by CV1 and CV2 determined by the value of ACFGT. ACFE must be
...
/** Compare Function Range Enable Mask Shift. */
...
/** DMA Enable Mask Shift. */
#define ADC_SC2_DMAEN_SHIFT                      2  
/**
 * Voltage Reference Selection (1-0 bits).
 * Selects the voltage reference source used for conversions.
 * 
 * Value | Status
 * :----:|:------
 * 00    | Default voltage reference pin pair, that is, external pins V REFH and V REFL. 
 * 01    | Alternate reference pair, that is, V ALTH and V ALTL . This pair may be additional external pins or internal sources depending on the MCU configuration. See the chip configuration information for details specific to this MCU.
 * 10    | Reserved.
 * 11    | Reserved.
 * 
 */
#define ADC_SC2_REFSEL_MASK                      0x3u
/*** Voltage Reference Selection Mask Shift. */
#define ADC_SC2_REFSEL_SHIFT                     0
#define ADC_SC2_REFSEL(x)                        (((uint32_t)(((uint32_t)(x))<<ADC_SC2_REFSEL_SHIFT))&ADC_SC2_REFSEL_MASK)
/** @} */
/* End of SC2 Bit Fields */

/**
    \addtogroup KL25Z_ADC_Register_SC3 A/D Status and Control Register 3(ADC0_SC3)
    ...
    continuous convert, and hardware averaging functions of the ADC module.    
    @{
*/
/* SC3 Bit Fields */
/**
 ...
 * the calibration is in progress and is cleared when the calibration 
 * sequence is completed. CALF must be checked to determine the result
 * of the calibration sequence. Once started, the calibration routine 
 * cannot be interrupted by writes to the ADC registers or the results 
 * will be invalid and CALF will set. Setting CAL will abort any current
 * conversion.
 */
#define ADC_SC3_CAL_MASK                         0x80u
/** Calibration Mask Shift. */
#define ADC_SC3_CAL_SHIFT                        7
/**
 * Calibration Failed Flag (6 bit). 
 * Displays the result of the calibration sequence. The calibration 
 * sequence will fail if SC2[ADTRG] = 1, any ADC register is written, 
 * or any stop mode is entered before the calibration sequence 
...
/** Calibration Failed Flag Mask Shift. */
#define ADC_SC3_CALF_SHIFT                       6
/**
 * Continuous Conversion Enable (3 bit).
...
/** Continuous Conversion Enable Mask Shift */
#define ADC_SC3_ADCO_SHIFT                       3
/**
 * Hardware Average Enable (2 bit). 
...
/** Hardware Average Enable Mask Shift */
#define ADC_SC3_AVGE_SHIFT                       2
/** 
 * Hardware Average Select (1-0 bits).
...
/** Hardware Average Select Mask Shift. */
#define ADC_SC3_AVGS_SHIFT                       0
#define ADC_SC3_AVGS(x)                          (((uint32_t)(((uint32_t)(x))<<ADC_SC3_AVGS_SHIFT))&ADC_SC3_AVGS_MASK)
/** @} */
/* End of SC3 Bit Fields. */

/**
    \addtogroup KL25Z_ADC_Register_OFS ADC Offset Correction Register (ADC0_OFS)
    \brief Defines for the bit fields in the \ref ADC0_OFS register. 
    The ADC Offset Correction Register (OFS) contains the user-selected 
    or calibration-generated offset error correction value. This 
    register is a 2’s complement, left-justified, 16-bit value . The 
    value in OFS is subtracted from the conversion and the result is 
    transferred into the result registers, Rn. If the result is greater
    than the maximum or less than the minimum result value, it is forced
    to the appropriate limit for the current mode of operation.
    @{
*/
/* OFS Bit Fields */
/** Offset Error Correction Value (15-0 bits). */
#define ADC_OFS_OFS_MASK                         0xFFFFu
/** Offset Error Correction Value Mask Shift. */
#define ADC_OFS_OFS_SHIFT                        0
#define ADC_OFS_OFS(x)                           (((uint32_t)(((uint32_t)(x))<<ADC_OFS_OFS_SHIFT))&ADC_OFS_OFS_MASK)
/** @} */
/* End of OFS Bit Fields */

/**
    \addtogroup KL25Z_ADC_Register_PG ADC Plus-Side Gain Register (ADC0_PG)
    ...
    for the plus-side input in differential mode or the overall
    conversion in single-ended mode. PG, a 16-bit real number in binary
    format, is the gain adjustment factor, with the radix point fixed 
    between ADPG15 and ADPG14. This register must be written by the user
    with the value described in the calibration procedure. Otherwise,
    the gain error specifications may not be met. 
    @{
*/
/* PG Bit Fields */
/** Plus-Side Gain (15-0 bits). */
#define ADC_PG_PG_MASK                           0xFFFFu
/** Plus-Side Gain Mask Shift. */
#define ADC_PG_PG_SHIFT                          0
#define ADC_PG_PG(x)                             (((uint32_t)(((uint32_t)(x))<<ADC_PG_PG_SHIFT))&ADC_PG_PG_MASK)
/** @} */
/* End of PG Bit Fields */

/**
    \addtogroup KL25Z_ADC_Register_MG ADC Minus-Side Gain Register (ADC0_MG)
    ...
    for the minus-side input in differential mode. This register is 
    ignored in single-ended mode. MG, a 16-bit real number in binary
    format, is the gain adjustment factor, with the radix point fixed 
    between ADMG15 and ADMG14. This register must be written by the user
    with the value described in the calibration procedure. Otherwise,
    the gain error specifications may not be met.
    @{
 */
/* MG Bit Fields */
/** Minus-Side Gain (15-0 bits). */
#define ADC_MG_MG_MASK                           0xFFFFu
/** Minus-Side Gain Mask Shift. */
#define ADC_MG_MG_SHIFT                          0
#define ADC_MG_MG(x)                             (((uint32_t)(((uint32_t)(x))<<ADC_MG_MG_SHIFT))&ADC_MG_MG_MASK)
/** @} */
/* End of MG Bit Fields. */
...
/** @} */ 
/* end of group KL25Z_ADC_Register_Masks */
...
/**
    \addtogroup KL25Z_ADC_Register_Accessor_Macros ADC - Register accessor macros
    \brief Here is the ADC register offset definitions for direct access.
    @{
*/
...
/** ADC0 Control Gate Control Mask Shift. */
 ...
 * \brief Optional system timer, SysTick
 * 
 * If implemented, when enabled, the timer counts down from the reload 
 * value to zero, reloads (wraps to) the value in the SYST_RVR on the 
 * next clock cycle, then decrements on subsequent clock cycles. Writing
 * a value of zero to the SYST_RVR disables the counter on the next 
 * wrap. When the counter transitions to zero, the COUNTFLAG status bit
 * is set to 1. Reading SYST_CSR clears the COUNTFLAG bit to 0.Writing
 * to the SYST_CVR clears the register and the COUNTFLAG status bit to 0.
 * The write does not trigger the SysTick exception logic. Reading the 
 * register returns its value at the time it is accessed.
 * 
 * \note When the processor is halted for debugging the counter does 
 * not decrement.
 * 
 ...
 * :---------:|:-----------|:----:|:-----------:|:-----------
 * 0xE000E010 | SYST_CSR   | RW   | See desc.   | SysTick Control and Status Register
 ...
 * 0xE000E01C | SYST_CALIB | RO   | See desc.   | SysTick Calibration Value Register
 * 
 * @{
*/
...
/** Enables the counter mask shift. */
...
/** Enable exception request mask shift. */
 ...
 * SYST_CSR.CLKSOURCE bit reads-as-one and ignores writes.
 */ 
...
#endif  /* #if !defined(MCU_KL25Z) */

#endif /* OpenKL25Z.h, eof. */
