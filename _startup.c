...
/**
    \addtogroup KL25Z_Initialization
    @{
*/
...
/**
    ...
    \note  RESET_PIN_CFG is set to enable external RESET signal
*/
...
/**
    ...
    \note  Generated by Processor Expert, cleaned up by hand. For 
    detailed information on clock modes, see the ""KL25 Sub-Family 
    Reference Manual" section 24.5.3.1.
*/
...
/**
 * \brief Blink function for onboard RGB LED.
 * \param pattern A mask of 32-bit with pattern to blink.
 * \param timing Delay, in us, between each bit shift of pattern.
 */
void blink (uint32_t pattern, uint32_t timing)
{
    uint32_t i;
    for(i=0; i!=32; i++) 
    {
        RGB_LED(pattern & 1 ? 100 : i * 3, i * 3, i * 3);           // Set RED led based on LSB
        pattern = (pattern >> 1) | (pattern << 31);     // Rotate
        delay(timing);
    }
}
...
/**
 * \brief Handle hard faults: print debugging information and halt.
 */
...
/**
 * \brief Initialize I/O pins for on-board RGB LED (PWM).
*/
 ...
 * The CPU reset vector points here. Initialize the CPU, and jump to 
 * the C runtime start, which will eventually invoke main().
 */
